...
def predict():
    ...
    for column in input_data.columns:
        unkown_categories = set(input_data[column]) - set(data[column].unique())
        if unkown_categories:
            input_data[column] =input_data[column].replace(unkown_categories,data[column].mode()[0])
